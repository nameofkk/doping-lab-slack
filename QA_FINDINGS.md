...
## AP3+AP4: 신규 풀생애주기 무게이트 + 안전·긴급정지
- AP3: 오토파일럿 ON이면 신규프로젝트의 planQuestions·승인플랜 마찰 스킵 → 기획~제작~QA~배포~등록 체이닝. logDecision(autopilot-newproject). 👤(키/계정)만 남김.
- AP4(AP1/2에 내장): "오토파일럿 꺼" 긴급정지(canCommand 불필요=킬스위치) + "오토파일럿 상태"(최근 자동실행 이력) + AP_BUILD_CAP 일일상한 + activeWork 동시성 + autopilot-run 로깅·OWNER DM + 작업중이면 게이트로 전환(폭주방지). 자기개선은 self=gate라 자동루프 안 생김.
- 검증: eval 통과. 라이브 오토파일럿 ON→"X 만들어줘" 무게이트 체이닝 / "오토파일럿 꺼" 즉시정지 확인.

## UX: 타이핑 연출 + 명령어 메뉴 + 게이트메시지 짤림 수정
- 타이핑 연출(replyTyping): 슬랙 네이티브 "작성 중"은 봇 불가(RTM 폐기·Socket Mode 미지원) → "💬 입력 중 ·/··/···" 임시메시지를 1초마다 순환(스피너 연출) 후 답으로 chat.update 교체. 잡담(targeted/random) 경로에 배선. 별도토큰 직원만, 아니면 폴백.
- 명령어 메뉴: commandMenuText()(카테고리별) + "명령어/도움말/메뉴/help" 텍스트 명령 + app.command(/도핑|dope/) 슬래시 핸들러(노션식 자동완성 — 단 Slack 앱설정에 슬래시 등록은 👤).
- 버그픽스: proposeOrAuto 게이트 메시지가 task.slice(0,36)로 단어 중간 짤리던 것 → 번호 리스트로 전체 표시.
- 검증: eval 통과. 라이브 타이핑 애니메이션·"명령어"·게이트 풀텍스트 확인.

## A1: 사업 메트릭 수집 (그로스 엔진 토대)
- bizData/biz.json: 서비스 stats 엔드포인트(키0 또는 👤 토큰)에서 curl→JSON→flattenNums(숫자만, 시크릿/문자열 제외)→일별 history(60일 롤링). 실데이터만(추정 0).
- 확인된 실엔드포인트: wewantpeace api.wewantpeace.live/public/stats(total_events·events_24h·active_clusters·monitored_countries·new_clusters_7d), /newsletter/stats(subscriber_count=58 실측). 기본 시드(키0 즉시). sponono는 api DNS 미설정→Railway URL/admin토큰(👤).
- 명령: "사업 메트릭 등록 <레포> <url> [이름] [헤더]", "사업 지표". 메뉴 추가.
- test/biz.mjs 11단언(flattenNums·파싱) + regress. 실제 curl로 wwp 실데이터 반환 확인(헤더 200 JSON, newsletter 58).
- 검증: eval 통과. 라이브 "사업 지표" → wwp 실수치 확인 필요.

## A2: 사업 브리핑 (AARRR 루브릭 해석)
- BIZ_RUBRIC(획득·활성화·리텐션·수익·추천·노스스타·경보기준선) 프롬프트 주입. runBizBriefing(): bizFetch 실수치+bizTrendLines(직전대비 증감)+루브릭 → LLM이 친한국어로 ①사업상태 ②특이사항 ③📍측정갭(안 보이는 핵심지표+계측법) ④타겟지표 단 개선안 1~3. 추정 0(미측정은 미측정).
- 데이터 wrapUntrusted 격리+scrubOutput. "사업 브리핑" 수동 + 일1회 자동(OPS_HOUR) + OWNER DM. 친한국어 라벨(BIZ_LABELS).
- 검증: eval 통과. 라이브 "사업 브리핑" → wwp 실수치 기반 AARRR 해석+측정갭(허위 0).

## A3: 그로스 루프 (제안→승인→실행→효과측정→학습)
- experiments.json + addExperiment(타겟키·baseline·가설). runBizGrowth(): 서비스별 스코어카드→LLM이 그로스 실험 1~2개(focus·target_key(측정가능 키)·가설·action) → addExperiment로 baseline 기록 → proposeOrAuto 게이트(프로드 build는 승인). "그로스 제안" 수동 + 주1회(화) 자동.
- measureExperiments(): 타겟키 baseline 대비 현재값 비교(효과), +10%면 스킬 학습(addSkill)·measured 마킹. "실험 현황" 명령.
- 모든 대화 스피너(startTyping/stopTyping) 적용. 가독성 스코어카드.
- 검증: eval 통과. 라이브 "그로스 제안"→타겟지표 단 실험 버튼, "실험 현황"→효과측정.

## A3-fix: 다중 제안 버튼 충돌 (위피 실행 눌렀는데 스포노노 실행)
- 증상(라이브): runBizGrowth가 서비스별 제안을 따로 올림 → 둘 다 pendingDispatch[channel] 단일슬롯 덮어씀 → 어느 버튼 눌러도 마지막(스포노노) 실행.
- 수정: 아이템에 repo 부착 → 한 제안으로 합침. dispatchActionItems가 byRepo 그룹별 처리(여러 서비스). proposeOrAuto apTier도 아이템별 repo. "실행 1,3" 선택 가능.
- 검증: eval 통과. 라이브 그로스 제안→한 제안에 위피/스포노노 섞여 나오고 골라 실행.

## Phase B + 버튼/출력 개선
- Phase B 부서 루프: DEPTS(cx·marketing·finance·market) + runDeptLoop 제네릭 — 부서별 실데이터 검토→진단+개선 제안(게이트). 페르소나=부서장. 웹서치 사용(useMcp). 명령 "고객 검토/마케팅 검토/재무 검토/경쟁 동향".
- 버튼: dispatchButtons(n) — 항목 여럿이면 개별 번호 버튼([전부 실행][1번만][2번만][넘어가], 슬랙 5개 한도). app.action에 dispatch_n<N>→"실행 N" 처리.
- 오토파일럿 출력 정리: 전체 덤프 대신 "자동 N건/승인 N건" 요약 + 게이트 리스트만.
- 검증: eval 통과. 라이브 그로스 제안 버튼 개별선택 + 부서 검토.

## 2+3 수정: 그로스 전부 게이트 + runReport repo 혼선
- 3번(repo 혼선 버그): runReport에서 안다연(반론자)·종합(한로로) runClaude가 cwd=WORKDIR(/app 봇 디렉토리)이라 클론한 레포가 아니라 봇 자기 코드를 봄("/app엔 index.js뿐" 정확히 이거). → cwd=dir(클론 레포)로 수정. 안다연이 실제 파일 검증 가능.
- 2번(그로스 게이트): proposeOrAuto에 forceGate 옵션. runBizGrowth·runDeptLoop는 forceGate:true → 오토파일럿 ON이어도 조사 포함 전부 승인받음(사업 변경은 신중).
- 검증: eval 통과.

## CX 인앱 피드백 연결 (wewantpeace)
- wewantpeace에 /admin/bot-feedback 엔드포인트 추가(봇전용, Feedback 모델) + bot-stats에 feedback_count. 실측: 피드백 2건("앱스토어 출시?" 문의, 팬메시지).
- 봇: bizFeedback(repo) curl + seedBizDefaults 피드백소스 시드. CX 부서루프가 인앱 피드백 우선 분석(외부스토어 추정 금지). 스코어카드에 인앱 피드백.
- 모노레포 배포 교훈 재확인: railway up과 git자동배포 레이스로 새코드 안 물 수 있음 → fresh clone railway up으로 강제, 검증.
- 검증: eval 통과. 라이브 고객 검토 → 인앱 피드백 기반 분석 확인.

## CX 스토어 리뷰 연결 (정확한 URL 주입)
- 스토어 라이브 확인: wewantpeace Play(com.wewantpeace.app, 리뷰 105개), sponono Chrome(lhbfibaioimpnmlcbhjmhekoidlfhhja, 게시됨). sponono Play(com.sponono.android)는 404=미게시.
- Play HTML 직접 파싱은 노이즈 많아 → STORE_URLS 정확한 URL을 CX 컨텍스트에 주입, 봇이 WebFetch로 해당 페이지 직접 읽어 평점·설치수·리뷰 정확히(엉뚱한 동명앱 방지). 공개데이터라 키 불필요.
- 인앱 피드백(bot-feedback) + 스토어 리뷰(WebFetch) 둘 다 CX가 분석.
- 설치 추이(일별)는 Play Console 키(👤) 필요 — 공개 페이지엔 근사 설치수만.
- 검증: eval 통과. 라이브 고객 검토 → 진짜 리뷰 기반 확인.
