...
async function runOppScout(client, channel, manual = false) {
  ...
  try {
    ...
    // Stage 3 — CEO 종합
    startTyping(channel);
    const synth = await runClaude(`너는 도핑연구소 CEO다. 아래 신사업 기회 후보와 팀 4인의 교차검증을 종합해, 교차포화를 뚫고 진짜 만들 가치 있는 것만 0~2개 골라라. 강한 게 없으면 빈 배열이 정답이야 — 억지로 채우지 마(약한 거 2개 올리느니 0개가 낫다).${GROUNDING_RULE}\n\n[후보]\n${list}\n\n[반증(안다연)]\n${refute}\n\n[재무·구현(윈터)]\n${fin}\n\n[획득(영듀)]\n${acq}\n\n[시장적합(아이유)]\n${mkt}\n\n[판정 규칙 — 엄격히]\n1) 해자(moat) 없으면 거의 버려라. 해자 '약'이면 confidence 최대 '중', '없음'이면 '낮음'.\n2) sources가 그 기회의 '구체적 수요'가 아니라 범용/인접 트렌드만 뒷받침하면 confidence '낮음'으로 내리고 demand_evidence에 그 사실을 적어라.\n3) 수익시뮬의 고객 수는 TAM 근거(출처)가 있으면 쓰고, 추정이면 숫자 뒤에 "(가정)"을 반드시 붙여라.\n4) 경쟁자 플랫폼 위에 짓거나(플랫폼 종속), 규제/책임 큰 업종이거나, 공공·무료 데이터라 해자가 없으면 risk_flags에 명시.\n5) 수익 천장(연 ARR)이 경쟁 강도 대비 너무 작으면(예: 거대자본 경쟁자 있는데 천장 작음) kill_risk나 confidence에 반영.\n\n각 선정안에 신뢰도, 한국/글로벌, 빌드 전 싼 검증 실험, 수익시뮬, 해자, 위험플래그, 가장 무서운 리스크, 트렌드 날짜·출처를 정리해라.\n\nJSON만(설명 금지): {"final":[{"title":"","market":"글로벌|한국|둘다","confidence":"상|중|낮음","moat":"강|중|약|없음 + 한 줄 근거","trend":"","trend_date":"","sources":["url"],"demand_evidence":"","ai_build":"","monetize":"","revenue_sim":"고객 N명(근거 or 가정)×가격=월매출","validation":"빌드 전 싼 검증 실험 한 가지","risk_flags":["플랫폼종속·규제책임·무해자 등 해당되는 것"],"kill_risk":"가장 무서운 반증 한 줄","needs":"우리가 필요한 것(키·데이터, 없으면 빈문자열)","rec":"validate|build"}]}`, MODEL.LEAD, WORKDIR, CLAUDE_PERMISSION_MODE, 360000, false);
    stopTyping(channel);
    ...
    const card = finals.map((c, i) => `${i + 1}. ${c.title}  (신뢰도 ${c.confidence || '?'} · 시장 ${c.market || '?'} · 해자 ${(c.moat || '?').split(' ')[0]} · 1차 ${c.rec === 'build' ? 'MVP 빌드' : '검증'})\n   트렌드(${c.trend_date || '시점미상'}): ${c.trend}\n   출처: ${(c.sources || []).slice(0, 3).join(' , ') || '(없음)'}\n   실수요: ${c.demand_evidence}\n   AI 구현: ${c.ai_build}\n   수익: ${c.monetize}${c.revenue_sim ? ` → ${c.revenue_sim}` : ''}\n   해자: ${c.moat || '-'}${c.risk_flags && c.risk_flags.length ? `\n   ⚠️ 경고: ${c.risk_flags.join(', ')}` : ''}\n   빌드 전 검증: ${c.validation || '-'}\n   제일 큰 리스크: ${c.kill_risk || '-'}${c.needs ? `\n   필요(너): ${c.needs}` : ''}`).join('\n\n');
    ...
    pendingOpp[channel] = { cands: finals, at: Date.now() };
...
async function runOppValidate(client, channel, cand) { // 한 기회의 수요를 WebSearch로 더 깊이 검증(레포 없음)
  ...
  try {
    ...
    startTyping(channel);
    const r = await runClaude(`${lead.prompt}${PLAIN}\n너는 지금 신사업 검증 리서처로서 아래 기회의 "진짜 수요와 수익성"을 WebSearch로 깊게 검증해. 낙관 금지, 반증도 적극적으로 찾아.${GROUNDING_RULE}\n\n[기회]\n제목: ${cand.title}\n트렌드: ${cand.trend}\n초기 수요근거: ${cand.demand_evidence}\nAI구현안: ${cand.ai_build}\n수익모델: ${cand.monetize}\n\nWebSearch로 확인할 것: (1) 실제 수요 크기·성장(검색추이·커뮤니티 규모·반복 불만) (2) 이미 있는 경쟁/대안과 그들의 약점 (3) 사람들이 돈을 내고 있는 증거(유료 대안·가격대) (4) 한국 vs 글로벌 어느 시장이 맞는지(한국이면 규제·결제·현지화) (5) 만들 때 걸림돌(데이터·API·규제·ToS). 확인하는 신호마다 언제 기준 데이터인지 시점과 출처 URL을 같이 적어. 마지막에 "지금 만들 가치: 강함/중간/약함 + 한 줄 이유", "빌드 전 싼 검증 실험 1가지", "만든다면 MVP 핵심 1가지". 마크다운 금지, 반말.`, MODEL.TEAM, WORKDIR, CLAUDE_PERMISSION_MODE, 360000, false);
    stopTyping(channel);
...
(async () => {
  ...
  loadRules();
  loadSettings();
  loadTasks(); loadJobs(); loadFacts(); loadSkills(); loadOntology(); loadSouls(); loadOpps(); buildMcpConfig(); loadDecisions(); loadUsage(); loadBiz(); loadExperiments(); loadPendingDispatch(); loadOpsConfig();
  loadLastRepo();
  loadServices();
  loadPending();
