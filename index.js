...
async function runOppScout(client, channel, manual = false) {
  ...
  try {
    ...
    // Stage 1 — 멀티모달 발굴
    const disc = await runClaude(`너는 신사업 기회 스카우트(리서처)다. WebSearch를 여러 각도로 여러 번 써서(구글 검색 트렌드 성장, Reddit/HackerNews 반복 불만, Product Hunt 신규 런칭, 펀딩 뉴스, 앱스토어 신규 카테고리) 지금 뜨는 트렌드 기반의 "AI 에이전트로 수익화 가능한 사업 기회" 후보 3~5개를 발굴해라.${GROUNDING_RULE}\n\n각 트렌드는 반드시 (a) 언제 기준 데이터인지 날짜/시점 (b) 실제 출처 URL을 명시해라(검색에서 직접 본 것만, 지어내지 마). 이미 하는 것(${known || '없음'})·최근 제안(${avoid || '없음'})과 겹치면 빼라.\n\nJSON만(설명 금지): {"cands":[{"title":"기회 한 줄","trend":"무슨 트렌드","trend_date":"이 트렌드 데이터 시점(예: 2026-06 검색 급증, 최근 3개월)","sources":["실제 URL 1~3개"],"demand_evidence":"실수요 근거(숫자·출처 포함)","ai_build":"AI 에이전트 구현안","monetize":"수익모델"}]}`, MODEL.TEAM, WORKDIR, CLAUDE_PERMISSION_MODE, 420000, false);
    stopTyping(channel);
    ...
    // Stage 3 — CEO 종합
    startTyping(channel);
    const synth = await runClaude(`너는 도핑연구소 CEO다. 아래 신사업 기회 후보와 팀 4인의 교차검증을 종합해, 교차포화를 뚫고 진짜 만들 가치 있는 1~2개만 골라라(반증이 치명적이면 버려).${GROUNDING_RULE}\n\n[후보]\n${list}\n\n[반증(안다연)]\n${refute}\n\n[재무·구현(윈터)]\n${fin}\n\n[획득(영듀)]\n${acq}\n\n[시장적합(아이유)]\n${mkt}\n\n각 선정안에 신뢰도, 한국/글로벌 권고, "빌드 전 싼 검증 실험"(랜딩+웨이트리스트 등), 대략 수익시뮬, 가장 무서운 리스크, 그리고 트렌드 날짜·출처를 정리해라.\n\nJSON만(설명 금지): {"final":[{"title":"","market":"글로벌|한국|둘다","confidence":"상|중","trend":"","trend_date":"","sources":["url"],"demand_evidence":"","ai_build":"","monetize":"","revenue_sim":"고객 N명×가격=월매출 식","validation":"빌드 전 싼 검증 실험 한 가지","kill_risk":"가장 무서운 반증 한 줄","needs":"우리가 필요한 것(키·데이터, 없으면 빈문자열)","rec":"validate|build"}]}`, MODEL.LEAD, WORKDIR, CLAUDE_PERMISSION_MODE, 360000, false);
    stopTyping(channel);
