...
async function gateReportFollowup(client, channel, thread_ts, repo, reportOut) {
  try {
    ...
    // 확정 전에 고치면 엉뚱한 데 손댐(grounding) — 사람 확인 단계가 있으면 그걸 먼저, 코드수정은 "원인 확정 후"로 프레이밍
    if (humans.length) await postAs(client, channel, thread_ts, LEAD, `🔎 고치기 전에 — 네가 먼저 확인해야 원인이 갈리는 것들이야(코드 밖이라 내가 못 봐):\n${humans.map((h, i) => `${i + 1}. ${h.task}`).join('\n')}\n이거부터 확인하고 원인 확정한 뒤에 아래 수정안 승인해(미리 고치면 진짜 원인은 안 고쳐질 수 있어).`);
    if (acts.length) { await proposeOrAuto(client, channel, acts[0].repo, acts, `${humans.length ? '원인 확정 후 권장 — ' : ''}조사 결과 다음 실행 제안 ("실행"/"실행 1,3", 버튼). 안 할 거면 "넘어가"`, { forceGate: true }); if (!humans.length) await postAs(client, channel, thread_ts, LEAD, '조사 결과 바탕으로 다음 실행안 위에 제안해놨어 — "실행"으로 승인하면 착수할게.'); }
  } catch (_) {}
}
