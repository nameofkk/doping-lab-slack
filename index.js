...
async function runWork(client, channel, thread_ts, repo, task, newProject, forcePR, projName) {
  ...
  await postAs(client, channel, undefined, LEAD, `${mention(channel)}${doneHead} ${forcePR ? '승인모드라 PR로 올렸어 (머지하면 반영).' : 'PR로 올렸어.'}\nPR: ${url}\n코드 브라우저로 보려면: https://github.dev/${repo}`); // 최종 결과 top-level
  if (summaryMsg) await postAs(client, channel, undefined, LEAD, `결과 요약\n${summaryMsg}`);
...
function loadRoadmap() { try { if (fs.existsSync(ROADMAP_FILE)) roadmap = JSON.parse(fs.readFileSync(ROADMAP_FILE, 'utf8')) || {}; } catch { roadmap = {}; } }
...
function addMilestone(repo, title, target, why, impact, effort) { if (!repo || !title) return null; roadmap[repo] = roadmap[repo] || []; if (roadmap[repo].some(m => m.title.toLowerCase() === String(title).toLowerCase() && m.status !== 'done')) return null; const m = { id: ++rmSeq + Date.now() % 1000, title: String(title).slice(0, 120), target: String(target || '').slice(0, 80), why: String(why || '').slice(0, 120), status: 'planned', impact: +impact || 3, effort: +effort || 3, rice: riceScore(impact, effort), at: Date.now() }; roadmap[repo].push(m); if (roadmap[repo].length > 40) roadmap[repo] = roadmap[repo].slice(-40); persistRoadmap(); return m; }
...
function loadBlockers() { try { if (fs.existsSync(BLOCKERS_FILE)) blockers = JSON.parse(fs.readFileSync(BLOCKERS_FILE, 'utf8')) || []; } catch { blockers = []; } }
function persistBlockers() { try { fs.writeFileSync(BLOCKERS_FILE, JSON.stringify(blockers.slice(-200))); } catch {} }
let blkSeq = 0;
function addBlocker(repo, what, kind) { if (!what) return null; const sig = String(what).toLowerCase().replace(/[^a-z0-9가-힣]/g, '').slice(0, 40); if (blockers.some(b => b.status === 'open' && b.sig === sig)) return null; const b = { id: ++blkSeq + Date.now() % 1000, repo: repo || null, what: String(what).slice(0, 200), kind: kind || 'todo', sig, at: Date.now(), status: 'open', lastNudge: Date.now() }; blockers.push(b); if (blockers.length > 200) blockers = blockers.slice(-200); persistBlockers(); return b; }
...
function loadWave4() { try { if (fs.existsSync(WAVE4_FILE)) { const j = JSON.parse(fs.readFileSync(WAVE4_FILE, 'utf8')) || {}; risks = j.risks || []; changelog = j.changelog || {}; ownerLastSeen = j.ownerLastSeen || 0; } } catch {} }
function persistWave4() { try { fs.writeFileSync(WAVE4_FILE, JSON.stringify({ risks: risks.slice(-100), changelog, ownerLastSeen })); } catch {} }
let riskSeq = 0;
function addRisk(repo, text, sev) { if (!text) return null; const sig = String(text).toLowerCase().replace(/[^a-z0-9가-힣]/g, '').slice(0, 40); if (risks.some(r => r.status === 'open' && r.sig === sig)) return null; const r = { id: ++riskSeq + Date.now() % 1000, repo: repo || null, text: String(text).slice(0, 160), sev: sev || '중', sig, at: Date.now(), status: 'open' }; risks.push(r); if (risks.length > 100) risks = risks.slice(-100); persistWave4(); return r; }
...
async function runPerformanceReview(client, channel) { // 어느 제안/출처가 실제 지표를 움직였나 — 메타 학습
  ...
  const hit = measured.filter(e => e.pct >= 10), bad = measured.filter(e => e.pct <= -10), meh = measured.length - hit.length - bad.length;
  const bySrc = {}; measured.forEach(e => { const s = e.source || '?'; bySrc[s] = bySrc[s] || { n: 0, h: 0 }; bySrc[s].n++; if (e.pct >= 10) bySrc[s].h++; });
