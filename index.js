...
function bumpUsage(j, limited) {
  try {
    const n = kstNow();
    if (usageStat.day !== n.day) { if (usageStat.day) usageHist.push(usageStat); if (usageHist.length > 30) usageHist = usageHist.slice(-30); usageStat = { day: n.day, calls: 0, outTokens: 0, limitedHits: 0, costUsd: 0 }; }
    usageStat.calls++; if (limited) usageStat.limitedHits++;
    const ot = j && j.usage && (j.usage.output_tokens || 0); if (ot) usageStat.outTokens += ot;
    // P1-3: 토큰 비용 추정 집계
    if (j && j.usage) {
      const modelKey = String(j.model || '').toLowerCase();
      const cost = Object.entries(MODEL_COST).find(([k]) => modelKey.includes(k));
      const rates = cost ? cost[1] : MODEL_COST['opus']; // 모르는 모델은 opus로 추정
      const inT = j.usage.input_tokens || 0;
      const outT = j.usage.output_tokens || 0;
      const usd = (inT * rates.input + outT * rates.output) / 1000000;
      usageStat.costUsd = (usageStat.costUsd || 0) + usd;
    }
...
async function proposeOrAuto(client, channel, repo, items, headerLine, opts) {
  ...
  // P3-12: 적응형 자율성 — 오토파일럿 꺼져 있어도 신뢰 점수 높으면 low/med 자동 실행
  if (!(opts && opts.forceGate) && (!settings.autopilot || !settings.autopilot[channel])) {
    const autoItems = items.filter(x => { const rl = riskTier(x.task); return shouldAutoApprove(channel, rl); });
    const gateItems = items.filter(x => { const rl = riskTier(x.task); return !shouldAutoApprove(channel, rl); });
    if (autoItems.length && !gateItems.length) {
      // 전부 자동 승인 가능 → 오토파일럿 ON처럼 실행
      await postAs(client, channel, undefined, LEAD, `${headerLine}\n신뢰도 기반 자동실행 ${autoItems.length}건 (승인률 높은 채널).`);
      if (!activeWork[channel]) { logDecision(channel, 'trust-auto-run', `자동실행 ${autoItems.length}건`); dispatchActionItems(client, channel, undefined, repo, autoItems).catch(() => {}); return; }
    }
    if (autoItems.length && gateItems.length) items = gateItems; // 자동 가능한 것은 바로 실행, 나머지만 게이트
    if (autoItems.length && !activeWork[channel]) { logDecision(channel, 'trust-auto-partial', `자동실행 ${autoItems.length}건`); dispatchActionItems(client, channel, undefined, repo, autoItems).catch(() => {}); }
  }
...
// ── P3-12: 적응형 자율성 — 채널별 신뢰 점수 기반 자동 승인 ──
const TRUST_FILE = process.env.TRUST_FILE || '/data/trust_scores.json';
let trustScores = {}; // {channelId: {score, approvals, rejections, lastUpdated}}
function loadTrustScores() { trustScores = loadJson(TRUST_FILE, {}) || {}; }
function persistTrustScores() { saveJson(TRUST_FILE, trustScores); }
function getTrustScore(channelId) {
  if (!trustScores[channelId]) trustScores[channelId] = { score: 50, approvals: 0, rejections: 0, lastUpdated: Date.now() };
  return trustScores[channelId];
}
function updateTrustScore(channelId, approved) {
  const ts = getTrustScore(channelId);
  if (approved) {
    ts.approvals++;
    ts.score = Math.min(100, ts.score + 3); // 승인 시 +3
  } else {
    ts.rejections++;
    ts.score = Math.max(0, ts.score - 8); // 거부 시 -8 (거부 가중치 높게)
  }
  ts.lastUpdated = Date.now();
  persistTrustScores();
}
function shouldAutoApprove(channelId, riskLevel) {
  const ts = getTrustScore(channelId);
  if (riskLevel === 'low' && ts.score > 80) return true;
  if (riskLevel === 'med' && ts.score > 60) return true; // medium은 아직 높은 신뢰 필요
  return false; // high risk는 항상 게이트
}
...
function addFact(key, text, source, ev) {
  ...
  persistFacts();
  // P2-6: 새 사실 추가 후 비동기 충돌 감지
  if (arr.length > 1 && source !== 'lesson' && source !== 'archive') detectMemoryConflict(key, t).catch(() => {});
}
// P2-6: 메모리 충돌 감지 — 같은 repo에 상충 fact가 있으면 Slack 경보
async function detectMemoryConflict(repo, newFact) {
  try {
    const arr = (facts[repo] || []).slice(-15);
    if (arr.length < 2) return;
    const existing = arr.map(f => f.text).join('\n');
    const r = await runClaude(`다음은 "${repo}" 프로젝트에 저장된 기존 사실(facts)과 새로 추가된 사실이야.
[기존 사실]
${existing}
[새 사실]
${newFact}
기존 사실 중 새 사실과 "모순·상충"되는 것이 있으면 JSON: {"conflict":true,"old":"상충하는 기존 사실 원문","reason":"왜 상충인지 한 줄"}
없으면: {"conflict":false}
JSON만:`, MODEL.FAST, WORKDIR, CLAUDE_PERMISSION_MODE, 30000, true);
    ...
    if (result.conflict) {
      ...
      if (defCh && botClient) {
        await botClient.chat.postMessage({ channel: defCh, text: scrubOutput(`[메모리 충돌 감지] ${repo}\n기존: ${result.old || '?'}\n신규: ${newFact}\n이유: ${result.reason || '?'}\n\n사람이 어느 쪽이 맞는지 결정 필요.`) }).catch(() => {});
      }
      try { log('warn', 'memory-conflict', { repo, reason: String(result.reason || '').slice(0, 80) }); } catch (_) {}
    }
  } catch (_) {}
}
...
// ── A: 헬스체크 다운 즉각 원인분석 (다운 감지 직후 비동기 호출) ──
// /health 응답 파싱 → 패턴 분류 → causeHistory 저장 → Slack 원인 보고.
// Railway 로그 대신 /health JSON 필드를 1차 근거로 사용 (빠름, 인증 불필요).
const DOWN_CAUSE_FIXES = {
  db_connection:       'DB 연결 불가 — Railway 볼륨 마운트·DB 서비스 상태·DATABASE_URL 환경변수 확인',
  db_query_overload:   'DB 쿼리 과부하 — 풀스캔 쿼리(COUNT(*) 등) 존재 여부, connection pool 크기 확인',
  redis_error:         'Redis 연결 불가 — Railway Redis 서비스 재시작·REDIS_URL 확인',
  worker_down:         'Celery 워커 다운 — Railway worker 서비스 재시작, beat 스케줄 확인',
  collection_stopped:  '수집 중단(events_24h=0) — 외부 API 키 만료·소스 차단·수집 워커 크래시 확인',
  overload_503:        '앱 과부하(503) — DB 연결 풀 소진 or 메모리 부족. 느린 쿼리·동시 요청 수 확인',
  upstream_502:        '업스트림 타임아웃(502) — 앱 처리 지연. 최근 배포 롤백 또는 메모리 증가 검토',
  process_crash:       'OOM/크래시(000) — Railway 로그에서 heap/OOM 에러 확인. 메모리 플랜 증가 검토',
  server_error:        '서버 내부 에러 — 최근 배포 커밋 로그·Railway 재배포 로그 확인',
  unknown:             '원인 불명 — Railway 대시보드에서 직접 로그 확인 필요',
};
async function diagnoseDown(client, channel, s) {
  if (!client || !channel || !s) return;
  const sre = byName('윈터') || LEAD;
  const repo = s.repo || '?';
  const downCode = s.downCode || '000';
  try {
    // 1. /health 응답 파싱 — 서비스가 healthUrl 또는 루트+/health 에서 응답하면 필드 기반 진단
    const healthUrl = s.healthUrl || (s.url ? String(s.url).replace(/\/$/, '') + '/health' : null);
    let healthBody = null;
    if (healthUrl) {
      try {
        const hr = await sh(`curl -s --max-time 6 -w "\\n%{http_code}" '${healthUrl.replace(/'/g, '')}' 2>/dev/null`);
        const ho = (hr.out || '').trim();
        const rawBody = ho.replace(/\d{3}\s*$/, '').trim();
        try { healthBody = JSON.parse(rawBody); } catch (_) {}
      } catch (_) {}
    }
    // 2. 원인 분류 (우선순위: /health 필드 > HTTP 에러코드)
    let cat = 'unknown', detail = '';
    if (typeof healthBody === 'object' && healthBody) {
      if (healthBody.db === 'error')                             { cat = 'db_connection';      detail = 'db: error'; }
      else if (healthBody.redis === 'error')                    { cat = 'redis_error';         detail = 'redis: error'; }
      else if (healthBody.worker_heartbeat === 'dead')          { cat = 'worker_down';         detail = 'worker_heartbeat: dead'; }
      else if (typeof healthBody.events_24h === 'number' && healthBody.events_24h === 0) { cat = 'collection_stopped'; detail = 'events_24h: 0'; }
    }
    if (cat === 'unknown') {
      if (downCode === '503')                   { cat = 'overload_503';    detail = 'HTTP 503'; }
      else if (downCode === '502')              { cat = 'upstream_502';    detail = 'HTTP 502'; }
      else if (downCode === '000')              { cat = 'process_crash';   detail = 'HTTP 000 (무응답)'; }
      else if (/^5/.test(downCode))            { cat = 'server_error';    detail = `HTTP ${downCode}`; }
    }
    // 3. causeHistory 저장 (C: 원인 이력 기억)
    if (!s.causeHistory) s.causeHistory = [];
    s.causeHistory = s.causeHistory.concat([{ at: Date.now(), code: downCode, cat, detail }]).slice(-10);
    persistServices();
    // 4. 반복 패턴 감지 — 최근 5회 중 같은 원인이 3회 이상이면 경고
...
const OPS_DEFS = { // id → 표시정보 + 기본값(주기/시각/요일)
  ...
  content_gen: { label: '콘텐츠 자동 생성+게시', desc: '서비스 지표+학습 기반 SNS 포스트 자동 생성(Threads 우선), API 키 없으면 Slack 초안 게시', defCad: 'daily', defHour: 11 }, // P1-4: CX 데이터 분석(10:30) 이후 실행
...
// P3-11: 인지 편향 감지 — OPS 실행 이력에서 특정 작업만 편향적 성공/실패 패턴 분석
function detectCognitiveBias() {
  try {
    const month = Date.now() - 30 * 86400000;
    const recentJobs = Object.values(jobs).filter(j => (j.updatedAt || 0) > month && /^(done|failed|cancelled|limited)$/.test(j.status));
    if (recentJobs.length < 10) return null; // 데이터 부족
    // OPS 유형별 성공/실패 비율
    const typeStats = {};
    for (const j of recentJobs) {
      const type = j.type || 'unknown';
      if (!typeStats[type]) typeStats[type] = { done: 0, failed: 0, total: 0 };
      typeStats[type].total++;
      if (j.status === 'done') typeStats[type].done++;
      if (j.status === 'failed' || j.status === 'limited') typeStats[type].failed++;
    }
    const biases = [];
    const overallRate = recentJobs.filter(j => j.status === 'done').length / recentJobs.length;
    for (const [type, stats] of Object.entries(typeStats)) {
      if (stats.total < 3) continue; // 최소 3건 이상
      const rate = stats.done / stats.total;
      // 전체 평균 대비 30%p 이상 편향
      if (Math.abs(rate - overallRate) > 0.3) {
        const dir = rate > overallRate ? '편향적 성공' : '편향적 실패';
        biases.push(`${type}: ${dir} (${Math.round(rate * 100)}% vs 전체 ${Math.round(overallRate * 100)}%, ${stats.total}건)`);
      }
    }
...
async function runWeeklyReport(client, channel, manual = false) {
  ...
  try {
    ...
    const kpiLines = [
      ...
      kpi.weekly_cost_usd != null ? `· 주간 추정 비용: $${kpi.weekly_cost_usd} (모델 호출 토큰 기준)` : null,
    ].filter(Boolean).join('\n');
    // P3-11: 인지 편향 감지
    const biases = detectCognitiveBias();
    const biasSection = biases && biases.length ? `\n\n[인지 편향 감지]\n${biases.join('\n')}` : '';
    // P1-5: error_rate 높을 때 원인 분석
    let errorAnalysis = '';
    if (kpi.error_rate != null && kpi.error_rate > 10) {
      try {
        const failedJobs = Object.values(jobs).filter(j => (j.updatedAt || 0) > week && j.status === 'failed').slice(-20);
        const failedOps = failedJobs.map(j => `${j.title || '?'} (${j.status})`).slice(0, 10);
        // 패턴 분석: 같은 제목의 연속 실패
        const titleCount = {};
        failedJobs.forEach(j => { const t = (j.title || '').split(' ').slice(0, 3).join(' '); titleCount[t] = (titleCount[t] || 0) + 1; });
        const repeats = Object.entries(titleCount).filter(([, c]) => c >= 2).sort((a, b) => b[1] - a[1]);
        const repeatStr = repeats.length ? repeats.slice(0, 3).map(([t, c]) => `"${t}" ${c}번 연속 실패`).join(', ') : '';
        errorAnalysis = `\n\n[오류 원인 분석 (error_rate ${kpi.error_rate}%)]\n실패 OPS ${failedJobs.length}건: ${failedOps.slice(0, 5).join(' / ') || '(상세 없음)'}${repeatStr ? `\n반복 패턴: ${repeatStr}` : ''}`;
        // 교훈 자동 기록
        if (repeatStr) addLesson('global', `OPS 반복 실패 패턴 감지: ${repeatStr.slice(0, 150)}`);
      } catch (_) {}
    }
    const r = await runClaude(`지난 주 반복 장애 패턴: ${top3}.\n미해결 blocker ${ob.length}건: ${blockerSummary}\n\n에이전트 KPI:\n${kpiLines || '(없음)'}${errorAnalysis}${biasSection}\n\n이번 주 봇 개선 제안 2~3개를 핵심만(반말, 각 1줄). 마지막에 사람 승인 필요한 blocker 요약.`, MODEL.LEAD);
    const text = deMd((r.text || '').trim());
    await postAs(client, channel, undefined, LEAD, `📊 주간 자율 리포트\n\n반복 패턴: ${top3}\n\n[에이전트 KPI]\n${kpiLines || '(집계 데이터 없음)'}${errorAnalysis}${biasSection}\n\n${text}\n\n${ob.length ? `⏳ blocker 큐 (${ob.length}건):\n${blockerSummary}` : '미해결 blocker 없음'}`).catch(() => {});
...
function computeAgentKPI() {
  ...
  // P1-3: 주간 토큰 비용 집계
  const weekDays = usageHist.filter(d => { try { const dt = new Date(d.day + 'T00:00:00+09:00'); return dt.getTime() > week; } catch { return false; } });
  const weekly_cost_usd = Math.round(((weekDays.reduce((s, d) => s + (d.costUsd || 0), 0) + (usageStat.costUsd || 0)) + Number.EPSILON) * 100) / 100;
  return { escalation_rate, content_performance, self_correction_rate, ops_success_rate, learning_velocity, error_rate, weekly_cost_usd };
}
...
const METRICS_QUEUE_FILE = process.env.METRICS_QUEUE_FILE || '/data/metrics_queue.json';
function loadMetricsQueue() { try { if (fs.existsSync(METRICS_QUEUE_FILE)) { const d = JSON.parse(fs.readFileSync(METRICS_QUEUE_FILE, 'utf8')); metricsQueue = Array.isArray(d) ? d : []; } } catch { metricsQueue = []; } }
function persistMetricsQueue() { try { saveJson(METRICS_QUEUE_FILE, metricsQueue); } catch (_) {} }
...
async function processMetricsQueue() {
  ...
  persistMetricsQueue();
  // P2-8: 성과 수집 후 게시 시간 최적화 분석 (데이터 충분할 때만)
  if (contentLog.filter(c => c.metrics).length >= 5) analyzePostingTimeOptimization().catch(() => {});
}
// P2-8: 최적 게시 시간 학습 — content_log에서 hour vs 성과 분석, OPS 시간 조정 제안
async function analyzePostingTimeOptimization() {
  try {
    const withMetrics = contentLog.filter(c => c.metrics && c.postedAt);
    if (withMetrics.length < 5) return; // 데이터 부족
    const hourBuckets = {};
    for (const c of withMetrics) {
      const d = new Date(c.postedAt + 9 * 3600000); // KST
      const h = d.getUTCHours();
      if (!hourBuckets[h]) hourBuckets[h] = { likes: 0, views: 0, count: 0 };
      hourBuckets[h].likes += (c.metrics.likes || 0);
      hourBuckets[h].views += (c.metrics.views || 0);
      hourBuckets[h].count++;
    }
    const ranked = Object.entries(hourBuckets)
      .filter(([, b]) => b.count >= 2)
      .map(([h, b]) => ({ hour: parseInt(h), avgLikes: Math.round(b.likes / b.count), avgViews: Math.round(b.views / b.count), count: b.count }))
      .sort((a, b) => b.avgLikes - a.avgLikes);
    if (!ranked.length) return;
    const best = ranked[0];
    const curHour = (opsConfig.content_gen && opsConfig.content_gen.hour) || 11;
    if (best.hour !== curHour && best.avgLikes > 0) {
      const defCh = settings.hqChannel || settings.monitorChannel || null;
      if (defCh && botClient) {
        const summary = ranked.slice(0, 4).map(r => `${r.hour}시: 평균 좋아요 ${r.avgLikes}, 조회 ${r.avgViews} (${r.count}건)`).join('\n');
        await botClient.chat.postMessage({ channel: defCh, text: scrubOutput(`[게시 시간 최적화 제안]\n현재 content_gen: ${curHour}시\n최고 성과 시간대: ${best.hour}시 (평균 좋아요 ${best.avgLikes})\n\n시간대별 성과:\n${summary}\n\n"운영 리듬"에서 content_gen 시간을 ${best.hour}시로 변경 검토.`) }).catch(() => {});
      }
      addFact('global', `SNS 최고 참여 시간대: ${best.hour}시(평균 좋아요 ${best.avgLikes})`, 'analytics');
    }
  } catch (_) {}
}
// ── P0-1: 숫자 환각 방지 게이트 — 콘텐츠 내 숫자가 실제 지표와 10% 이상 괴리면 차단 ──
function verifyContentNumbers(postText, repo) {
  try {
    const real = bizLatest(repo);
    if (!real) return { pass: true }; // 지표 없으면 검증 불가 → 통과
    const nums = (postText.match(/[\d,]+(\.\d+)?/g) || []).map(s => parseFloat(s.replace(/,/g, '')));
    if (!nums.length) return { pass: true };
    const realVals = Object.values(real).filter(v => typeof v === 'number' && v > 0);
    if (!realVals.length) return { pass: true };
    const issues = [];
    for (const n of nums) {
      if (n <= 0) continue;
      for (const rv of realVals) {
        // 같은 규모의 숫자만 비교 (자릿수 차이 2 이내)
        if (Math.abs(Math.log10(n || 1) - Math.log10(rv || 1)) > 2) continue;
        const pct = Math.abs(n - rv) / rv;
        if (pct > 0.10 && pct < 1.5) { // 10%~150% 오차 범위에서 유사 숫자 감지
          issues.push(`텍스트 숫자 ${n} vs 실제 지표 ${rv} (오차 ${Math.round(pct * 100)}%)`);
        }
      }
    }
...
// P2-7: 브랜드 규칙 ↔ 스킬 충돌 감지
async function detectRulesSkillsConflict(brandRules, repo) {
  try {
    if (!brandRules || !brandRules.length) return;
    const snsSkills = (skills[repo] || []).filter(s => s.tier === 'active' && /SNS|콘텐츠|마케팅|포스트/i.test(s.name + ' ' + s.when + ' ' + s.recipe));
    if (!snsSkills.length) return;
    const skillsStr = snsSkills.map(s => `${s.name}: ${s.recipe}`).join('\n');
    const rulesStr = brandRules.join('\n');
    const r = await runClaude(`다음 브랜드 규칙과 학습된 스킬(성공 패턴)을 비교해서 서로 충돌하는 게 있는지 확인해라.
[브랜드 규칙]
${rulesStr}
[학습된 스킬]
${skillsStr}
충돌이 있으면 JSON: {"conflict":true,"pairs":[{"rule":"규칙 요약","skill":"스킬 이름","reason":"왜 충돌인지"}]}
없으면: {"conflict":false}
JSON만:`, MODEL.FAST, WORKDIR, CLAUDE_PERMISSION_MODE, 30000, true);
    const m = ((r && r.text) || '').match(/\{[\s\S]*\}/);
    if (!m) return;
    const result = JSON.parse(m[0]);
    if (result.conflict && result.pairs && result.pairs.length) {
      const defCh = settings.hqChannel || settings.monitorChannel || null;
      if (defCh && botClient) {
        const pairs = result.pairs.slice(0, 3).map(p => `규칙: ${p.rule}\n스킬: ${p.skill}\n이유: ${p.reason}`).join('\n\n');
        await botClient.chat.postMessage({ channel: defCh, text: scrubOutput(`[브랜드 규칙 vs 스킬 충돌 감지] ${repo}\n\n${pairs}\n\n충돌하는 스킬을 격리하거나 브랜드 규칙을 수정해야 해.`) }).catch(() => {});
      }
      try { log('warn', 'brand-skill-conflict', { repo, count: result.pairs.length }); } catch (_) {}
    }
  } catch (_) {}
}
// 개선 3: 콘텐츠 게이트 대기 상태
...
- 마크다운 없이 평문으로${GROUNDING_RULE}`;
        ...
        // P0-1: 숫자 환각 방지 — 실제 지표 대조
        const numCheck = verifyContentNumbers(postText, repo);
        if (!numCheck.pass) {
          try { log('warn', 'number-hallucination', { repo, issues: (numCheck.issues || []).slice(0, 3).join('; ').slice(0, 120) }); } catch (_) {}
          addLesson(repo, `SNS 콘텐츠 숫자 환각: ${(numCheck.issues || [])[0] || '오차 초과'}`);
          const fixNum = await runClaude(`${prompt}\n\n[경고] 방금 생성한 포스트에 실제 지표와 10% 이상 다른 숫자가 있었어: ${(numCheck.issues || []).join(', ')}. 정확한 수치로 다시 생성해:`, MODEL.TEAM, WORKDIR, CLAUDE_PERMISSION_MODE, 60000, true);
          const fixedNum = ((fixNum && fixNum.text) || '').trim();
          if (fixedNum) {
            const reNumCheck = verifyContentNumbers(fixedNum, repo);
            if (reNumCheck.pass) postText = fixedNum;
            else { await postAs(client, channel, undefined, LEAD, `${name} 콘텐츠 숫자 오차 재생성 후에도 미해결. 수동 검토 필요.`).catch(() => {}); continue; }
          }
        }
        // 개선 1: 브랜드 규칙 준수 검증 (규칙 있을 때만)
        const brandRules = rules['__brand__'] || [];
        if (brandRules.length) {
          // P2-7: 브랜드 규칙 vs 스킬 충돌 비동기 감지 (콘텐츠 루프 진입 시 1회)
          detectRulesSkillsConflict(brandRules, repo).catch(() => {});
...
위반 없이 다시 생성:`;
        ...
        if (hasSocialKey('threads') && settings.contentGate !== false) {
          ...
          if (result.postId) {
            ...
            // P3-10: 게시 후 24h 뒤 조회수 DoD 검증
            const bl = bizLatest(repo); if (bl && typeof bl['admin.total_users'] === 'number') scheduleDoDCheck('content_gen', repo, 'admin.total_users', bl['admin.total_users'], channel, `SNS 콘텐츠 게시(${name})`, 24 * 3600000);
...
JSON만: {"results":[{"idx":1,"category":"bug|feature_request|complaint|praise|question","sentiment":"positive|negative|neutral","urgency":"high|medium|low","confidence":0~100},...]}
confidence는 분류 확신도(0~100). 70 미만이면 불확실한 것.${GROUNDING_RULE}`;
        ...
        // P3-9: 낮은 confidence 항목 에스컬레이션
        const lowConf = classified.filter(c => typeof c.confidence === 'number' && c.confidence < 70);
        if (lowConf.length) {
          const lcList = lowConf.slice(0, 5).map(c => {
            const fb = feedbacks[c.idx - 1]; const txt = String((fb && (fb.text || fb)) || '').slice(0, 80);
            return `#${c.idx} (${c.category}, 확신도 ${c.confidence}%): "${txt}"`;
          }).join('\n');
          await postAs(client, channel, undefined, LEAD, `[CX 분류 불확실] ${name}\n다음 ${lowConf.length}건은 분류 확신도가 낮아 사람 검토 필요:\n${lcList}`).catch(() => {});
          try { log('info', 'cx-low-confidence', { repo, count: lowConf.length }); } catch (_) {}
        }
        // 자동 처리 대상에서 낮은 confidence 제외
        const autoClassified = classified.filter(c => typeof c.confidence !== 'number' || c.confidence >= 70);
        const catCount = {};
        autoClassified.forEach(c => { catCount[c.category] = (catCount[c.category] || 0) + 1; });
...
// ── P3-10: DoD 사후 검증 — 작업 후 실제 지표 변화 확인 (24~48h 후) ──
const DOD_QUEUE_FILE = process.env.DOD_QUEUE_FILE || '/data/dod_queue.json';
let dodQueue = []; // [{taskType, repo, expectedMetric, baseline, checkAt, channel, description}]
function loadDodQueue() { try { if (fs.existsSync(DOD_QUEUE_FILE)) { const d = JSON.parse(fs.readFileSync(DOD_QUEUE_FILE, 'utf8')); dodQueue = Array.isArray(d) ? d : []; } } catch { dodQueue = []; } }
function persistDodQueue() { try { saveJson(DOD_QUEUE_FILE, dodQueue); } catch (_) {} }
function scheduleDoDCheck(taskType, repo, expectedMetric, baseline, channel, desc, delayMs) {
  dodQueue.push({ taskType, repo, expectedMetric, baseline, checkAt: Date.now() + (delayMs || 24 * 3600000), channel, description: desc || taskType });
  persistDodQueue();
}
async function processDoDQueue() {
  const now = Date.now();
  const due = dodQueue.filter(q => q.checkAt <= now);
  if (!due.length) return;
  dodQueue = dodQueue.filter(q => q.checkAt > now);
  persistDodQueue();
  for (const q of due) {
    try {
      const current = bizLatest(q.repo);
      if (!current) continue;
      const curVal = current[q.expectedMetric];
      const baseline = q.baseline;
      const ch = q.channel || settings.hqChannel || settings.monitorChannel;
      if (!ch || !botClient) continue;
      if (typeof curVal === 'number' && typeof baseline === 'number' && baseline > 0) {
        const pctChange = Math.round(((curVal - baseline) / baseline) * 100);
        if (pctChange > 5) {
          // 개선됨
          addFact(q.repo, `DoD 달성: ${q.description} 후 ${q.expectedMetric} ${baseline}→${curVal} (+${pctChange}%)`, 'verify');
          addSkill(q.repo, `${q.taskType} 효과 확인`, `${q.description} 실행 시`, `${q.expectedMetric} ${pctChange}% 개선 확인됨`);
          await botClient.chat.postMessage({ channel: ch, text: scrubOutput(`[DoD 검증 성공] ${q.repo.split('/').pop()}\n${q.description}\n${q.expectedMetric}: ${baseline} -> ${curVal} (+${pctChange}%)`) }).catch(() => {});
        } else if (pctChange < -5) {
          // 악화
          addLesson(q.repo, `DoD 미달: ${q.description} 후 ${q.expectedMetric} 오히려 ${pctChange}% 악화`);
          await botClient.chat.postMessage({ channel: ch, text: scrubOutput(`[DoD 검증 실패] ${q.repo.split('/').pop()}\n${q.description}\n${q.expectedMetric}: ${baseline} -> ${curVal} (${pctChange}%) — 기대와 반대 결과. 접근 재검토 필요.`) }).catch(() => {});
        } else {
          // 미미한 변화
          await botClient.chat.postMessage({ channel: ch, text: scrubOutput(`[DoD 검증 미미] ${q.repo.split('/').pop()}\n${q.description}\n${q.expectedMetric}: ${baseline} -> ${curVal} (${pctChange > 0 ? '+' : ''}${pctChange}%) — 유의미한 변화 없음.`) }).catch(() => {});
        }
      }
    } catch (_) {}
  }
  ...
  for (const s of list) {
    ...
    if (code === '000' || code === '403') {
      ...
      for (const ru of railUrls) {
        ...
        if (fm && /^2\d\d|^3\d\d/.test(fm[1])) { dnsIssue = true; originCode = fm[1]; s.railwayUrl = ru; break; } // 원본 정상이어도 code/ms/size는 공개 URL 값 유지(up 안 뒤집음)
      }
    }
...
위를 반영해서 Threads 포스트 재생성 (500자 이내, 마크다운 없이, 포스트 텍스트만 출력):${GROUNDING_RULE}`;
    ...
    if (pendingDispatch[channel]) {
      if (pendingDispatch[channel].at && Date.now() - pendingDispatch[channel].at > 30 * 60 * 1000) { delete pendingDispatch[channel]; } // 30분 만료
        ...
        updateTrustScore(channel, false); // P3-12: 거부 시 신뢰 점수 하락
        ...
        updateTrustScore(channel, true); // P3-12: 승인 시 신뢰 점수 상승
...
(async () => {
  ...
  loadRules();
  loadSettings();
  loadTasks(); loadJobs(); loadFacts(); loadSkills(); loadOntology(); loadSouls(); loadOpps(); loadRoadmap(); loadBlockers(); loadWave4(); buildMcpConfig(); loadDecisions(); loadUsage(); loadBiz(); loadExperiments(); loadPendingDispatch(); loadOpsConfig(); loadCI(); loadCooldowns(); loadPendingOpp(); loadContentLog(); loadMetricsQueue(); loadDodQueue(); loadTrustScores();
  loadLastRepo();
  loadServices();
  loadPending();
  ...
  setInterval(() => {
    ...
    // P3-10: DoD 사후 검증 큐 처리
    if (dodQueue.length) processDoDQueue().catch(() => {});
