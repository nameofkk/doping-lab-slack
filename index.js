...
const LOTTIE_RULE = 'Lottie(벡터 모션) — 게임/히어로뿐 아니라 "상태·순간"이 있는 모든 화면에 적용 검토: 로딩/스켈레톤, 빈 상태(데이터 없음 일러스트), 성공·완료 체크, 에러·실패, 온보딩 단계 안내, 결제/제출 완료 축하, 빈 검색결과, 404·오류 페이지, 알림 아이콘, 버튼 마이크로인터랙션, 차트 등장. 적용법: (a) 임베드는 @lottiefiles/dotlottie-react(React)나 lottie-web(바닐라). (b) JSON은 "npx skills add diffusionstudio/lottie" 스킬로 생성하거나 lottiefiles.com 무료/CC0 받아 써(출처·라이선스 ASSETS.md 기록). (c) 남용 금지 — 한 화면에 임팩트 한두 개만, prefers-reduced-motion 존중, lazy-load(뷰포트 진입 시), 전부 autoplay 금지. 단순 트랜지션은 CSS로, 복잡한 일러스트 모션만 Lottie. 무거운 GIF/MP4 대신.';
const DESIGN_RULE = `
